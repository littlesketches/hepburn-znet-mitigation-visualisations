    ...
    // Initialisation function to pull data from google sheet
      ...
      function drawMACC(d, levelisedCost,animationTime) {   
        ...
        let totalAbatement = d3.sum(d, function (d) {return d.abatementEnergy} );         

        x.domain([0, targetEnergy] );
        y.domain(d3.extent(d, function (d) {return d.levelisedCostEnergy}) );
        ...
        svg.selectAll(".block").data(d)
          ...
          .attr("y", y(0))  // Start all on y = zero prior to transitioning
            .attr("width", function(d) { return x(d.abatementEnergy)})
            .attr("height", 0)
          ...
          .transition().ease(d3.easeLinear)
            ...
            .delay(function(d, i) { return  i * animationTime / blockIDs.length;  })
              .attr("y", function(d){ return d.levelisedCostEnergy > 0 ? y(d.levelisedCostEnergy) : y(0); })  // Offset positive blocks by height to position correctly on y-axis
              .attr("height", function(d) { return Math.abs(y(d.levelisedCostEnergy) - y(0)); })                
        ...
        let xAxisGroup = svg.append("g")
            ...
            xAxisGroup.append("text")
              ...
              .style("text-anchor", "end")
              .text("GJ abated or sustainably sourced per annum");
    ...
    // Tooltip for blocks
      function toolTipOn(d, x, y, levelisedCost) {
        tooltipDiv  
          .html("<span class= 'tooltipHeader'>"+ d.name +"</span><p class = 'tooltipText'>Contribution to ZNET Energy: "+ Math.round(d.abatementEnergy / baselineEnergy * 1000)/10 +"%</p><p class = 'tooltipText'>Emissions abatement: "+formatComma(d.abatementEnergy) +" GJ p.a. </p><p class = 'tooltipText'>Cost per GJ: $"+d['levelisedCostEnergy'] +" per GJ </p><p class = 'tooltipText'>About this option:<br>"+d.description +"<br>" )
          .style("left", (x) + "px")   
          .style("top", (y + 10) + "px")
    ...
    ///////////////////////
      ...
      function parseData(d){
        targetEnergy =  +d[0]['targetEnergy']
        baselineEnergy =  +d[0]['baselineEnergy']

        for(i = 0; i < d.length; i++){
          let obj = d[i];
          obj.levelisedCostEnergy = +obj.levelisedCostEnergy; 
          obj.abatementEnergy = +obj.abatementEnergy; 
        ...
        d.sort(function(a,b) {return (a.levelisedCostEnergy > b.levelisedCostEnergy) ? 1 : ((b.levelisedCostEnergy > a.levelisedCostEnergy) ? -1 : 0);} ); 
        ...
        for(i = 0; i < d.length; i++){
          ...
          cumAbatement = cumAbatement + obj.abatementEnergy;
        };
